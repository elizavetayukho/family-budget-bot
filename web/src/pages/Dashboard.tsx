...
export default function Dashboard() {
  ...
  const requesterPerson = state.lizaveta;
  ...
  // Edgar = total minus Liz (safe for 2-person system, transfers cancel out)
  function personJarRows(person: 'lizaveta' | 'edgar') {
    return state!.sharedJars.map(jar => {
      if (person === 'lizaveta') {
        return { jar, contribution: jar.myContribution, spending: jar.mySpendingShare, balance: jar.myBalance };
      }
      return {
        jar,
        contribution: jar.totalContribution - jar.myContribution,
        spending: jar.totalSpending - jar.mySpendingShare,
        balance: jar.balance - jar.myBalance,
      };
    });
  }

  function PersonTab({ person }: { person: 'lizaveta' | 'edgar' }) {
    const p = state![person];
    const rows = personJarRows(person);
    const isLiz = person === 'lizaveta';
    return (
      <div className="space-y-4">
        {/* Income breakdown card */}
        <div className="bg-white rounded-2xl p-5 space-y-3" style={{boxShadow:'0 2px 16px 0 rgba(124,58,237,0.07)'}}>
          <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Income breakdown</p>
          <div className="space-y-1.5 text-sm">
            <div className="flex justify-between">
              <span className="text-gray-500">Income</span>
              <div className="flex items-center gap-2">
                <span className="font-semibold text-brand-900">{fmtPln(p.income)}</span>
                {(p.incomeSource === 'estimated' || p.incomeSource === 'brutto') && (
                  <span className="text-xs bg-amber-100 text-amber-700 px-2 py-0.5 rounded-full font-medium">
                    {p.incomeSource === 'estimated' ? 'Estimated' : 'Brutto'}
                  </span>
                )}
              </div>
            </div>
            <div className="flex justify-between">
              <span className="text-gray-500">Overheads (50%)</span>
              <span className="font-medium text-red-400">−{fmtPln(p.overheadShare)}</span>
            </div>
            {p.personalDeductions > 0 && (
              <div className="flex justify-between">
                <span className="text-gray-500">Personal deductions</span>
                <span className="font-medium text-red-400">−{fmtPln(p.personalDeductions)}</span>
              </div>
            )}
            <div className="flex justify-between font-semibold border-t border-brand-100 pt-2 text-brand-900">
              <span>Discretionary</span>
              <span>{fmtPln(p.discretionary)}</span>
            </div>
          </div>
        </div>

        {/* Jar contribution table */}
        <div className="bg-white rounded-2xl overflow-hidden" style={{boxShadow:'0 2px 16px 0 rgba(124,58,237,0.07)'}}>
          <div className="px-5 pt-4 pb-2">
            <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide">Jar breakdown</p>
          </div>
          <div className="divide-y divide-brand-50">
            {rows.map(({ jar, contribution, spending, balance }) => {
              const spendPct = contribution > 0 ? Math.min(100, (spending / contribution) * 100) : 0;
              return (
                <button key={jar.id} onClick={() => setActiveJar(jar)}
                  className="w-full text-left px-5 py-3.5 hover:bg-brand-50 transition-colors">
                  <div className="flex items-center justify-between mb-1.5">
                    <span className="text-sm font-medium text-brand-900">{jar.name}</span>
                    <span className={`text-sm font-semibold tabular-nums ${balance < 0 ? 'text-red-500' : 'text-brand-700'}`}>
                      {fmtPln(balance)}
                    </span>
                  </div>
                  <div className="h-1.5 bg-brand-100 rounded-full overflow-hidden mb-1.5">
                    <div className={`h-1.5 rounded-full transition-all ${balance < 0 ? 'bg-red-400' : 'bg-brand-500'}`}
        ...
        {/* Personal jar — only for Liz (requesting user), not exposed for Edgar */}
        {isLiz && (
          <div className="gradient-card rounded-2xl p-5 text-white">
            <p className="text-xs font-medium text-white/80 uppercase tracking-wide mb-1">Personal jar</p>
            <p className="text-3xl font-bold tabular-nums">{fmtPln(p.personalJarBalance)}</p>
            <p className="text-xs text-white/70 mt-1">Only visible to you</p>
          </div>
        )}
      </div>
    );
  ...
  return (
    <div className="max-w-5xl mx-auto p-4 sm:p-6 space-y-4 sm:space-y-5">
      ...
      {/* Tabs */}
      <div className="flex gap-1 bg-brand-50 rounded-2xl p-1">
      ...
      {activeTab === 'overview' && (
        <>
            ...
            {state.sharedJars.map((jar, idx) => {
              ...
              return (
                <button key={jar.id} onClick={() => setActiveJar(jar)}
                  ...
                  <p className={`text-xs font-semibold uppercase tracking-wide mb-2 ${isFeatured ? 'text-white' : 'text-gray-500'}`}>
                    {jar.name}
                  </p>
                  ...
                  <div className={`flex justify-between text-xs mt-2 ${isFeatured ? 'text-white/80' : 'text-gray-500'}`}>
                    <span>Your share: <span className={isFeatured ? 'text-white font-medium' : (jar.myBalance < 0 ? 'text-red-400 font-medium' : 'text-brand-600 font-medium')}>{fmtPln(jar.myBalance)}</span></span>
                    <span>Total: <span className={isFeatured ? 'text-white font-medium' : 'text-brand-900 font-medium'}>{fmtPln(jar.totalSpending)}</span> spent</span>
                  </div>
                  {jar.carryForward !== 0 && (
          ...
          <div className="gradient-card rounded-2xl p-5 text-white">
            <p className="text-xs font-medium text-white/80 uppercase tracking-wide mb-1">Personal jar</p>
            <p className="text-3xl font-bold tabular-nums">{fmtPln(requesterPerson.personalJarBalance)}</p>
            <p className="text-xs text-white/70 mt-1">Only visible to you</p>
          </div>
        </>
      )}

      {activeTab === 'lizaveta' && <PersonTab person="lizaveta" />}
      {activeTab === 'edgar' && <PersonTab person="edgar" />}
